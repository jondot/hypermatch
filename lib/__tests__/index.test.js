...
const t = ['or', ['or', ['includes', 'name', ['a']], ['equals', 'browser', 'firefox']], ['includes', 'uid', ['user1']]];

const t1 = ['and', ['includes', 'name', ['a']], ['equals', 'browser', 'firefox']];

const bad = ['and', ['incluuudes', 'name', ['a']], ['equals', 'browser', 'firefox']];
...
const testRun = (res, tree, props) => {
  test(`${JSON.stringify(tree)} ${res ? '✓' : '✕'} ${JSON.stringify(props)}`, () => {
...
test('handles bad syntax', () => expect(() => {
  run(bad, { name: 'a' });
}).toThrow());
